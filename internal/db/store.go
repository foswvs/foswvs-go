...
// Store wraps the SQLite database with prepared statements and mutex protection.
type Store struct {
	db *sql.DB
	mu sync.RWMutex
}
...
func (s *Store) UpsertDevice(mac, ip, hostname string) (int64, error) {
	...
	var id int64
	err := s.db.QueryRow("SELECT id FROM devices WHERE mac_addr = ?", mac).Scan(&id)
	if err == sql.ErrNoRows {
		res, err := s.db.Exec(
			"INSERT INTO devices(mac_addr, ip_addr, hostname) VALUES(?, ?, ?)",
			mac, ip, hostname,
		)
		if err != nil {
			return 0, err
	...
	_, err = s.db.Exec(
		"UPDATE devices SET ip_addr=?, hostname=?, topup_count=0, updated_at=CURRENT_TIMESTAMP WHERE id=?",
		ip, hostname, id,
	)
	return id, err
}
...
func (s *Store) GetDeviceIDByIP(ip string) (int64, error) {
	...
	var id int64
	err := s.db.QueryRow(
		"SELECT id FROM devices WHERE ip_addr = ? ORDER BY updated_at DESC LIMIT 1", ip,
	).Scan(&id)
	if err == sql.ErrNoRows {
		return 0, nil
	}
...
func (s *Store) GetDeviceMAC(deviceID int64) (string, error) {
	...
	var mac string
	err := s.db.QueryRow("SELECT mac_addr FROM devices WHERE id = ?", deviceID).Scan(&mac)
	return mac, err
}
...
func (s *Store) GetDataUsage(deviceID int64) (DataUsage, error) {
	...
	err := s.db.QueryRow(
		"SELECT SUM(mb_limit), SUM(mb_used) FROM session WHERE device_id = ?", deviceID,
	).Scan(&limit, &used)
	if err != nil {
		return du, err
	}
...
func (s *Store) UpdateMBUsed(deviceID int64, mb float64) error {
	...
	_, err := s.db.Exec(
		`UPDATE session SET mb_used = mb_used + ?, updated_at = CURRENT_TIMESTAMP
...
func (s *Store) AddSession(deviceID int64, amount, mbLimit float64) (int64, error) {
	...
	res, err := s.db.Exec(
		"INSERT INTO session(device_id, piso_count, mb_limit) VALUES(?, ?, ?)",
		deviceID, amount, mbLimit,
	)
	if err != nil {
		return 0, err
	}
...
func (s *Store) GetTopupCount(deviceID int64) (int, error) {
	...
	var count int
	err := s.db.QueryRow("SELECT topup_count FROM devices WHERE id = ?", deviceID).Scan(&count)
	return count, err
}
...
func (s *Store) IncrTopupCount(deviceID int64) error {
	...
	_, err := s.db.Exec(
		"UPDATE devices SET topup_count = topup_count + 1, topup_at = CURRENT_TIMESTAMP WHERE id = ?",
		deviceID,
	)
	return err
}
...
func (s *Store) GetAllTransactions(offset, limit int) ([]Transaction, error) {
	...
	rows, err := s.db.Query(
		`SELECT s.piso_count, s.mb_limit,
		        ...
		        d.mac_addr, d.ip_addr, IFNULL(d.hostname,'-NA-')
		 FROM session s LEFT JOIN devices d ON s.device_id = d.id
		 ORDER BY s.id DESC LIMIT ? OFFSET ?`, limit, offset,
	)
	if err != nil {
		return nil, err
	}
...
func (s *Store) AddShareTx(deviceID int64, token string) error {
	...
	_, err := s.db.Exec("INSERT INTO sharetx(device_id, token) VALUES(?, ?)", deviceID, token)
	return err
}
